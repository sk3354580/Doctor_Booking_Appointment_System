...
def sorted(request):
    if request.method == "POST":
        gen = request.POST.getlist('male')
        specs = request.POST.getlist('dspecs')
        print(specs)
        print(gen)
        query = doctor_profile.objects.filter(Q(Gender__in=gen)|Q(Specialiazation__in=specs) )
        return render(request,'practice.html',{'query':query})
    else:
        return render(request,'search.html')
    ###############time schewdule close
...
def patientprofile(request,id):
    sel_pat = patient_profile.objects.get(patient=id)
    print(sel_pat.id)
...
def reviews(request,rid):
    ...
    if request.method == "POST":
        ...
        r.save()
        return HttpResponse('saved successfull')


    else:
        return render(request, 'practice.html',{'rid':rid})
